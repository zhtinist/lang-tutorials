...
反例：如果 `Order` 这个 Entity 直接用 SQLAlchemy 的 `@Column` 装饰字段、`ApplyDiscountUseCase` 直接调用 `flask.request.get_json()` 取参数——想给 `Order.discount()` 这条打折规则写一个单元测试，就必须先起一个 Flask 应用上下文、连上一个真实（或内存）数据库，测试跑起来又慢又脆弱；某天团队决定把 MySQL 换成 Postgres，或者把 Web 框架从 Flask 换成 FastAPI，改动会直接波及 Entity 和 Use Case 这两层本该最稳定的代码。而按依赖规则写的版本——`Order` 只做纯粹的业务计算、`ApplyDiscountUseCase` 只依赖 `OrderRepository` 这个抽象接口——`ApplyDiscountUseCase.execute()` 可以脱离任何 Web 框架和真实数据库直接调用测试，换数据库也只需要新写一个 `OrderRepository` 实现，不动业务代码一行。
