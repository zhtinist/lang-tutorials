...
打开一个网站，你能看到文章、看到用户信息、能点赞——这些看得见摸得着的东西，REST 把它们统一看作**资源**：每个资源有个网址（相当于门牌号），你用不同的动作（看 / 发布 / 修改 / 删除）对它做操作，而不是给每个动作单独发明一套地址规则。
...
例如「给一篇文章点赞」：客户端对 `/posts/1` 发一个 `POST /posts/1/likes` 请求，服务器把这篇文章在数据库里的点赞数从 100 改成 101——这就是一次「状态转化」，状态本身（点赞数）存在服务器上，客户端只是通过 HTTP 动词触发了这次变化，然后拿到一份新的表现层（比如 `{"id": 1, "likes": 101}` 这段 JSON）作为响应，而不是自己去直接操作数据库。
...
反例：如果客户端代码里写死了「查完 zoo 之后，动物列表一定在 `/zoos/{id}/animals`」，某天后端把动物列表迁到了独立的 `/animal-management/zoos/{id}/animals` 路径下，所有硬编码这个 URL 的客户端都会瞬间 404，需要各自发版修复；而如果客户端一直是从上一步响应里的 `link.href` 拿到下一步的地址（HATEOAS 的做法），后端换路径时只需要改自己返回的链接值，客户端代码完全不用动。这也是为什么完整 HATEOAS 虽然实现成本高，但确实能换来客户端与服务端路径解耦。
